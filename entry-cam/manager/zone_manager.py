...
Usage
-----
    zm = ZoneManager(config.ENTRY_ZONES)

    # Each frame per active track:
    event = zm.update(cid, (cx, cy))
    # Returns "detector->inside", "inside->detector", or None
...
Zone setup
----------
Run tools/draw_zones.py once against your camera to determine coordinates,
then paste the output into config.py as ENTRY_ZONES.
"""
...
class ZoneManager:
    ...
    def update(self, cid: int, centre: Tuple[int, int]) -> Optional[str]:
        ...
        Returns crossing string e.g. "detector->inside" or None.
