"""
EntryDB  —  Session Gallery
============================
Persistent deduplication store for the entry pipeline.
...
------
  ...
  first_entry  REAL                 unix timestamp of first ever crossing
  last_seen    REAL                 unix timestamp of most recent sighting
  entry_count  INTEGER              total confirmed unique crossings

Design
------
P6  Entire table loaded into RAM at startup. All mid-frame lookups are
    pure numpy matmul -- zero DB reads during the frame loop.

FIX2  find_match() enforces two conditions before returning a match:
        a) best cosine similarity >= threshold (raised to 0.88 in config)
...
class EntryDB:
    ...
    def __init__(self, db_path: str = _DEFAULT_DB_PATH):
        ...
        self._first_entries: Dict[int, float]      = {}
    ...
    # ── Startup cache load (P6) ───────────────────────────────────────────────

    def _load_cache(self) -> None:
        cur = self._conn.execute(
            "SELECT cid, embedding, image_path, first_entry, entry_count "
            "FROM session_gallery"
        )
        for cid, blob, image_path, first_entry, count in cur.fetchall():
            emb = np.frombuffer(blob, dtype=np.float32).copy()
            self._embeddings[cid]    = emb
            self._entry_counts[cid]  = count
            self._first_entries[cid] = first_entry
            self._image_paths[cid]   = image_path or ""
        print(f"[EntryDB] Loaded {len(self._embeddings)} person(s) from session gallery.")

    # ── Lookup (FIX2: threshold + gap check) ─────────────────────────────────
    ...
    def find_match(self, embedding: np.ndarray, threshold: float) -> Optional[int]:
        ...
        from causing false suppression of a genuine new entry.
        """
        if not self._embeddings:
            return None

        ids   = list(self._embeddings.keys())
        mat   = np.stack(list(self._embeddings.values()), axis=0)   # (N, 290)
        ...
        # Condition 1: must exceed threshold
        if best_sim < threshold:
            return None

        # Condition 2: must be clearly the best (FIX2 gap check)
        if len(sims) > 1:
    ...
    def upsert(
        ...
        if cid in self._embeddings:
            ...
            self._entry_counts[cid]  = 1
            self._first_entries[cid] = now
            self._image_paths[cid]   = image_path

            self._conn.execute(
                """INSERT INTO session_gallery
                   (cid, embedding, image_path, first_entry, last_seen, entry_count)
                   VALUES (?, ?, ?, ?, ?, 1)""",
    ...
    def reset(self) -> None:
        ...
        self._first_entries.clear()
        self._image_paths.clear()
        print("[EntryDB] Session gallery reset.")

    def total_unique(self) -> int:
        return len(self._embeddings)

    def entry_count(self, cid: int) -> int:
        return self._entry_counts.get(cid, 0)
